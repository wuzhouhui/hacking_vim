...
\textbf{BSD} (\textbf{Berkeley Software Distribution}) 的一个早期版本.
vi 是当时最流行的编辑器 \textbf{ex} 的扩展版本. 而 ex 则是 Unix 编辑器 
\textbf{ed} 的扩展版本. ``vi'' 的意思是 \textbf{visual in ex}, 正如这个
名字所表示的那样, vi 仅仅是一个命令, 命令的作用是以可视化模式启动 ex 
编辑器.

vi 是最早的引入\textbf{模式} (\textbf{modality}) 概念的编辑器之一.
...
为 Unix 社区最流行的编辑器之一, 这主要是因为 \textbf{SUS} (\textbf{Single
Unix Specification}) 把 vi 列为 Unix 系统的必备软件之一 --- 只有符合 
SUS 的系统才能称之为 Unix 系统.

\subsection{STEVIE}
\label{subsec:stevie}
1987 年, Tim Thompson 获得了他的第一台 \texttt{Atari ST}
(\textbf{Sixteen/Thirty-two}), 但这个平台还没有一款优秀的编辑器可供
使用, 于是, 它决定把 vi 移值到这个平台中. 1987 年 6 月, 他发布了一款
编辑器, 所使用的授权类似于后来的开源协议. 他在新闻组中发布了这款编辑器,
并取名为 \textbf{STEVIE} --- 意为 \textbf{ST Editor for VI Enthusiasts}.
...
\section{nvi}
\label{sec:nvi}
\textbf{nvi}, 全称 \textbf{new vi}, 是 AT\&T 与 加州大学伯克利分校 
\textbf{Computer Science Research Group} (\textbf{CSRG}) 授权争论的
...
\section{Vim}
\label{sec:vim}
\newterm{Vim} 编辑器是 vi 家庭的天之骄子. 自从 Bram Moolenaar 在 1991 年
