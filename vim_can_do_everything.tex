...
虽然 Vim 只是一个文本编辑器, 但仍然有很多人开发了大量的脚本, 让 Vim 完成
其他一些非编辑任务, 其中包括可以直接在 Vim 中玩的小游戏. 这些小游戏不是那些
类似于 ``20 个小问题'' 那样的文本类游戏, 而是带有图形界面的. 它们由 ASCII
字符组成, 虽然不是很完美, 但对于游戏来说已经足够了.
\marginpar{202}

\subsection{生命游戏}
\label{subsec:game_of_life}

第一个要介绍的游戏严格说来并不能算作游戏, 但是仍然值得说一下. 生命游戏 (
...
样可以通过脚本实现. 对于前面提到的三个版本控制系统, 我推荐以下几个脚本:
\begin{itemize}
...
去不停地更新所有的东西. 如果用户移动到另一个和 IRC 客户端无关的缓冲区, 循环
