...
\marginpar{95}
\begin{warning}
    用户如果希望 Vim 把所有的设置信息, 比如光标的位置与折叠信息, 那么就需要在
    \texttt{vimrc} 中添加如下内容:
    \begin{vimcode}
    ...
    \end{vimcode}
    无论何时在同一个窗口打开另一个缓冲区, 之前的缓冲区的视图都会被自动保存下来,
    并且当再次打开缓冲区时, 会自动还原保存的视图.
\end{warning}

会话的一个常用技巧是定义一个命令, 功能是在退出 Vim 时自动保存会话, 而在打开 Vim
...
有时候, 用户可以拥有同一文件的多个版本 --- 这些版本之间可能相同, 也可能不同.
...
件存放在服务器家目录下的 \texttt{public_html/}. 使用 Vim 打开此文件的命令是:
\begin{vimcode}
