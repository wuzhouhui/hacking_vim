...
那么, 怎么才能创建一个模版文件? 不妨让我们用 HTML 文件作为例子来进行讲解.
...
再把模版写得更完善一点: 添加对占位符 (占位符指的是你将要添加文本的地方) 的
...
:autocmd BufNewFile * silent! 0r $VIMHOME/templates/%:e.tpl
\end{vimcode}
让我们对这个命令进行修改, 使得 Vim 可以根据文件的类型, 为模式模版自动加
载适当的缩写.
...
录下, 把文件命名为 \path{FILETYPE.snippet} (\path{FILETYPE} 表示文件类型,
...
现在来看一个补全函数的例子. 假设你有一个通讯录文件, 文件的内容是人名及其邮件
地址, 就像:
\begin{vimcode}
...
现在我们已经准备好了完整的宏, 光标的位置也已就位, 这意味着现在可以重复地
...
供了一个特性用来帮助用户解决这个问题 --- 折叠文本块. 这一节介绍如
利用折叠来更方便地获取文本内容的概览.
...
netrw (Net Read/Write) 的系统, 它可以用来帮助用户编辑远程服务器上的文件,
通过例子来介绍如何使用它.
