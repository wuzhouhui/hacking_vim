...
通过这些选项, 用户几乎可以设置与缩进相关的方方面面. 但是这些还不足够, 用户仍
...
后一种方法比如简单. 先是切换到可视模式, 比如按下 \key{Shift+v}, 然后再按
\key{=} 来重新缩进被选中的代码行.
...
我们还介绍了文本的对齐, 还解释了纯文本编辑器在对齐方面的困难. 接下来, 我们创
了一个函数, 该函数用于标记标题行, 以及生成无编号与编号列表. 我们还看到了 Vim
非常的灵活, 比如用户可以告诉 Vim 是否需要对选中的文本中的每一行执行操作, 又或
者是是否让用户来处理, 并待处理的行一次都反馈给用户的函数.

接下来, 我们介绍在 Vim 中如何格式化代码, 尤其是缩进. 我们认识因为每个人都有
着自己独特的编码风格, 所以很难有一个通用的功能来完成代码的格式化. Vim 提供了
一个灵活的接口, 使用用户可以按照自己的需要来设置. 我们还介绍了一些用来迅速格
