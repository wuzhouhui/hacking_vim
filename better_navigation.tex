...
如果用户已经很接近待搜索单词的某次出现, 但它并非你所想要的那次聘. 或者是你想要
...
\texttt{vimgre}, 而对于 Vim 帮助系统, 则用的是下面这个命令:
\begin{vimcmd}
:helpgrep pattern [@LANG]
\end{vimcmd}
命令需要一个必填参数 --- 你所想要搜索的模式 --- 还一个是可选参数, 用于限定语
言, 让我们来看一个例子. 比如说用户需要一些关于自动实例的帮助信息, 但是不知道
...
正如你所看到的那样, 放置符号的命令有点写起来有点麻烦, 但是我们可以把它映射到一
个键上.
...
时, 符号就会被放置到当前行.
\begin{center}
    \includegraphics[scale=0.7]{./images/page69.png}
\end{center}
...
光标就会马上跳转到该行的开始 (如果该行是缩进过的, 那就是该行的第一个非空白字
符).
...
导航的方式有许多种, 再接下来的小节我们学习到了通过搜索, 不仅可以在打开的文件内
导航, 甚至包括磁盘上的文件. 我们还学习了如何在 Vim 帮助系统搜索某个主题的相
关帮助信息.

最后, 我们学习了如何通过符号与标记在文件间跳转, 以及当我们使用 Vim 时, 它如何
自动地帮助我们添加标记.
