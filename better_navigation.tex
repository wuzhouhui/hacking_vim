...
\marginpar{53}
同时应付多个文件可能是一件非常麻烦的事, 有时候, 用户可能会花更多的时间去
定位文件, 而不是编辑.
...
Vim 不希望用户在移动光标上花费太多的时间, 虽然用户可以通过方向键遍历字母, 从
...
如果是注释块, 则不会被括号所包围, 因此 Vim 也就无法利用括号跳转到块的开始或结
束.
...
对笔者个人来说, 我更倾向于回绕显示, 因为这样可以更方便地看到文本的全貌. 不过这有时会
...
假如用户希望在按住 \key{Alt} 的同时, 再用方向键 上/下 移动光标, 那么 Vim
就应该按照视觉上的行 --- 而不是实际的行 --- 进行响应. 为了完成这样的效果, 需要在
...
可能就无法用单个按键表示 \texttt{]}, 在这种情况下, 重新映射按键会比较好. 在一
...
退格键完成. 如果在 Vim 帮助系统中也加上这个功能, 那就再好不过了. 下面这个映射
可以用来完成这个功能:
\begin{vimcode}
nmap <buffer> <BS> <C-T>
\end{vimcode}
...
这种打开缓冲区的方法并不总是最高效的. 用户还可以用下面这两个命令循环遍历所有
的缓冲区:
\begin{itemize}
    \item \texttt{:bnext}
...
如果可以用一个简便的命令打开被包含的文件, 那就太棒了. Vim 的确提供了这样的
...
用户知道在单词 \texttt{someWord} 的附近有自己感兴趣的内容, 为了找到它, 可以
在普通模式下执行:
\begin{vimcode}
?someWord
\end{vimcode}
该命令在文件内向后搜索问号右边的单词的第一次出现. 如果光标当前是在文件的末尾,
用它搜索单词正好合适. 然而, 如果当前光标是在文件的开始, 那么向前搜索可能会
...
到单词 \texttt{foo} 上, 再按下 \texttt{g\#}, 如果上文含有单词 \texttt{foobar}
与 \texttt{food}, 光标就会陆续跳转到这两个单词上.
...
双星号通配符 \texttt{**} --- 比如 \texttt{**/*.c} --- 搜索当前目录中的所有 C 文
件, 并递归搜索子目录.
\begin{center}
...
言, 先看一个例子. 比如说用户需要一些关于自动补全的帮助信息, 但是不知道
...
实际上, 命令 \texttt{helpgrep} 并没有遍历所有的文档, 而是使用了 tag list 对模式进
行搜索. Tag list 并不会自动生成, 所以, 如果用户安装的插件带有自己的文档, 那就必须
使用下面的命令:
\begin{vimcode}
...
在 Vim 中, 可以用一个可见的标记 --- 符号 --- 标记一行. 一个符号指的是一
个标记, 它会显示在编辑器最靠左的列中.
...
\end{center}
有时候, 用户想要移除符号, 用术语来说, 叫作 ``unplace'' 一个符号:
\begin{vimcmdform}
...
再接下来的小节我们学习了如何快速地在缓冲区之间导航, 以及如何通过两次击键,
打开被另一个文件所引用的文件.
