...
为了能让 Vim 准确识别出 \textit{endpattern}, 我们必须严格遵守.
...
\begin{enumerate}
	...
	\texttt{python statements here} \\
	\textit{endpattern}
	\end{vimcmdform}
	它们执行 \texttt{endpattern} 之间的所有 Python 代码.
...
Vim 对 Ruby 的支持非常完美, 因此用户可以在 Vim 中运行 Ruby 代码, 执行的方式
...
脚本需要检查某个设置是否可用, 而且不能破坏用户已经的设置.

我们学习了如何在脚本作用域内使用变量与函数, 这样就避免了污染全局作用域.
通过限定作用域, 可以确保用户只能访问到和他们相关的功能.
% FIXME
% Again this was to help the user such that only the relevant functionality
% is available to him or her.

在查看了几个 Vim 脚本的结构之后, 我们介绍了如何检查一些先决条件, 比如操作系统
与 Vim 版本, 以便在继续执行之前, 判断某些需求是否满足.
\marginpar{200}

学习了如何创建 Vim 脚本之后, 我们介绍了如何调试 Vim 脚本. 这一节介绍了 Vim 的
调试模式, 以及如何逐行地执行代码.

脚本编写完成并且没有错误后, 接下来就可以发布脚本了. 这一节介绍了如何发布脚本,
这样的话, 其他人就可以安装并使用你的脚本.

文档是已发布脚本的非常重要的一部分, 这一节介绍了如何使用 Vim 标记来编写文档.
通过使用这些标记, Vim 的帮助系统就可以检索到我们自己编写的文档, 而且在文档
中跳转也很方便.
...
我们还介绍了这三种语言都提供了用于和 Vim 交互的模块, 这些模块可以访问 Vim 的
许多信息, 因此开发人员可以直接在这些语言脚本中控制 Vim.

看到这里, 读者就有能力自己编写 Vim 脚本, 调试它们并发布. 在附录里, 我们将会
看到某些 Vim 用户开发的脚本几乎可以让 Vim 完成任意的事情.
