...
通过使用他的脚本, 并按照他的语法创建 \texttt{vimrc}, 用户不仅可以使新的配置
...
\begin{center}
  ...
  \hline
   \texttt{"}	& 开始一个注释 \\
   \texttt{ID}	& 一个标识符, 脚本可以用它区分不同的配置组. 当脚本启动时,
     用户要告诉脚本应该处理哪个配置组 \\
...
\end{vimcode}
用户还可以利用它插入空行, 这些空行将作为间距使用, 方法是用空格替换掉文本.
...
记住, 用户还可以用 \texttt{Nread} 和 \texttt{Nwrite} 这两个函数直接修改
在线 \texttt{vimrc}.
