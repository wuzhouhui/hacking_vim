...
简单而言, 语法高亮的关键是识别出文本中的特定单词与结构, 然后再给它们设置对
...
如果仅仅是根据单词与符号进行匹配, 那也可以得到一个相当不错的结果. 下面是具体
...
只是对单个单词进行匹配, 所以注释中的相同单词也会被匹配. 这种配置方法很难区分
代码与注释.
\marginpar{143}

那么, 我们可以从这个简单的例子里学习哪些东西呢? 那就是, 与高亮相比, 更重要的
...
前面的例子是用选项 \texttt{match} 选择注释语句. 在某些情况下,
...
安装完 Vimball 脚本后, 就可以用它安装其他 Vim 脚本.
...
安装目录, 就把命令换成:
\begin{vimcmdform}
\texttt{:UseVimball} \textit{path}
\end{vimcmdform}
\marginpar{150}
\textit{path} 是脚本的安装路径. 需要注意的是, 有些脚本只能安装在
...
如果用户记得安装时所用的 Vimball 文件名, 就可以通过它卸载脚本. 卸载 Vimball
的命令是:
\begin{vimcmdform}
...
之前所有的, 通过 Vimball 安装的脚本 (当然, 还可以通过手工删除进行卸载).
...
\begin{vimcmdform}
\texttt{:let myvar =} \textit{value}
\end{vimcmdform}
命令中的 \textit{value} 取决于变量的具体类型. 对于字符串与数字, 定义的方式与
前面讲过的相同, 比如:
\begin{vimcode}
...
\phantom{4em}\textit{code-to-execute-if-condition-is-met} \\
\texttt{endif}
\end{vimcmdform}
如果 \textit{condition} 的值为真, \texttt{if} 和 \texttt{endif} 之间的语句就
会执行, 否则就不执行.

那么, 在 \texttt{if} 结构中可以使用哪些条件? 主要有两种 --- 使用逻辑
运算符的语句, 与使用字符串运算符的语句. 现在看一下这两种运算符如何使用. 其
总的形式都是:
\begin{vimcmdform}
\texttt{value1} \textit{operator} \texttt{value2}
\end{vimcmdform}
命令中的 \textit{operator} 是用于比较 \texttt{value1} 与 \texttt{value2} 的运
算符. 比如:
\begin{vimcode}
value1 >= value2
\end{vimcode}
...
\begin{vimcmdform}
\texttt{if} \textit{condition} \\
\phantom{4em}\textit{code-to-execute-if-condition-is-true} \\
\texttt{else} \\
\phantom{4em}\textit{code-to-execute-if-condition-is-NOT-true} \\
\texttt{endif}
\end{vimcmdform}
\marginpar{159}
...
\begin{vimcmdform}
\texttt{if} \textit{condition1} \\
\phantom{4em}\textit{code-to-execute-if-condition1-is-true} \\
\texttt{else} \\
\phantom{4em}\texttt{if} \textit{condition2} \\
\hspace*{4em}\textit{code-to-execute-if-condition2-is-true} \\
\phantom{4em}\texttt{endif} \\
\texttt{endif}
\end{vimcmdform}

可以看到, \textit{condition1} 与 \textit{condition2} 中只有一个的值可以为真,
...
\texttt{if} \textit{condition1} \\
\phantom{4em}\textit{code-to-execute-if-condition1-is-true} \\
\texttt{elseif} \textit{condition2} \\
\phantom{4em}\textit{code-to-execute-if-condition2-is-true} \\
\texttt{endif}
\end{vimcmdform}
...
\begin{warning}
    ...
    右边的线性表拼接到左边的线性表上, 比如: \texttt{:let mylistvar4 +=
    [5,6,7,8,]}
\end{warning}

如果待拼接的线性表只含有一个元素, 那么它的效果与添加一个新元素是相同的.

除了运算符 \texttt{+}, 还可以用函数 \texttt{extend()} 完成拼接. 它的一个
例子是:
\begin{vimcode}
...
\marginpar{163}
这个关键词告诉 Vim 把该函数当成一个字典函数, 并对一个特殊的变量做操作 ---
...
\begin{vimcmdform}
\texttt{for} \textit{var} \texttt{in} \textit{ranges} \\
\phantom{4em}\textit{do-something} \\
\texttt{endfor}
\end{vimcmdform}
在上面的形式中, 循环依次遍历每一个元素, 每次迭代都会更新变量 \textit{var} 的
值, 一个例子是:
\begin{vimcode}
...
\end{vimcode}
上面的命令使用了函数 \texttt{range()} 生成从 1 到 10 的整数, 循环从 1 开始
...
\phantom{4em}\textit{execute-this-code} \\
\texttt{endwhile}
\end{vimcmdform}
...
函数的定义形式是:
\begin{vimcmdform}
\texttt{function} \textit{Name}\texttt{(}\textit{arg1}\texttt{,}
\textit{arg2}\texttt{,} \textit{...argN}\texttt{)} \textit{keyword} \\
\phantom{4em}\textit{code-to-execute-when-function-is-called} \\
\texttt{endfunction}
\end{vimcmdform}
\textit{Name} 是函数的名字, 函数名以字母开始, 并且只能包含字母, 数字, 下划线.

\textit{arg1} 到 \textit{argN} 是函数的参数, 它们是调用函数时必须提供的东西.
...
函数参数的个数存放在一个特殊的变量中 --- \texttt{a:0}, 所以用它判断应该
在什么时候停止循环.
...
用户可能希望把所有的参数当作一个线性表进行访问, Vim 提供了这个功能, 这个特殊的
...
\begin{vimcmdform}
\texttt{:delfunction} \textit{function-name}
\end{vimcmdform}
其中, \textit{function-name} 是用户想要删除的函数的名字.
...
这一章主要是为那些想通过脚本扩展 Vim 功能的人而写. 本章开始先介绍了一种最简单
