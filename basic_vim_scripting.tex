...
为了访问到每一个变量, 我们把 \texttt{argnum} 当作索引, 用于访问可选参数列表,
...
\end{vimcode}
\marginpar{172}
这次, 我们是通过 \texttt{for} 循环, 把可选参数一个接一下地传递给 \texttt{arg},
...
之后, 我们讨论如何在实际上使用脚本文件, 介绍了几种 Vim 可用的脚本类型, 以及在
哪些可以获取到它们. 在这一节结束时, 我们介绍了脚本的安装方法与卸载方法.
...
既然我们已经知道了如何安装以及如何使用脚本, 是时候介绍如何开发 Vim 脚本.

我们首先介绍了 Vim 脚本的基本变量类型, Vim 脚本变量几乎是无类型的 (只有两种基本
类型). 然而, 我们仍然可以利用这些类型存放各种各样的变量.

在介绍完 Vim 脚本语言的基本结构之后, 我们开始介绍如何创建函数.
\marginpar{173}
我们学习到, 通过一个关键词, Vim 就会知道应该如何调用函数. 这就允许我们将函数
绑定某种特定的变量上, 比如字典变量. 于是, 我们就能定义一些可以直接对字典变量
进行操作的函数.

在学习完这一章之后, 在下一章, 我们将会介绍如何运用本章所学的知识, 写出一个
结构良好的脚本.
% end of chapter 6
